...
const VoiceAssistant = () => {
  ...
  useEffect(() => {
    return VoiceCommandService.subscribe((status) => {
      ...
      } else if (status.type === "tts-loading") {
        setTtsLoading(!!status.loading);
      }
    });
  }, []);
  ...
  const handleToggle = useCallback(() => {
    if (isListening) {
      ...
      VoiceCommandService.speak(
        "Listening. Say help for a list of available commands.",
        false,
        false,
      );
    }
  ...
  const displayCommands = Array.from(
    new Map(
      buildCommandsForView(currentView, { activeHomeTab }).map((c) => [
        c.label,
        c,
      ]),
    ).values(),
  ...
  // Keyboard focus management: focus microphone button on mount for screen reader users
  const micButtonRef = useRef(null);
  useEffect(() => {
    if (micButtonRef.current) {
      micButtonRef.current.setAttribute('tabindex', '0');
    }
  }, []);
    ...
    >
      ...
      {ttsLoading && (
        <div
          ...
          aria-label="Loading Kokoro voice engine"
          role="status"
        >
          <div style={{ marginBottom: '0.7rem' }}>
            <svg width="38" height="38" viewBox="0 0 38 38" xmlns="http://www.w3.org/2000/svg" stroke="#dc2626">
          ...
          <div style={{ fontWeight: 600, color: '#dc2626', fontSize: '1.05rem', marginBottom: '0.2rem' }}>
            Loading voice engine…
          </div>
          <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Kokoro TTS is starting up</div>
        </div>
      )}
        ...
        >
          ...
          >
            <span
              ...
              id="va-commands-title"
            >
              Voice Commands
            </span>
            <button
              onClick={() => setShowHelp(false)}
              onKeyDown={e => handleKeyDown(e, () => setShowHelp(false))}
              style={{
                background: "none",
                border: "none",
                cursor: "pointer",
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }} aria-labelledby="va-commands-title">
      ...
      <div style={{ display: "flex", gap: "0.5rem", pointerEvents: "all" }}>
        {/* Help button (keyboard accessible) */}
        <button
          onClick={() => setShowHelp((s) => !s)}
          onKeyDown={e => handleKeyDown(e, () => setShowHelp((s) => !s))}
          style={{
            width: "40px",
            height: "40px",
            borderRadius: "9999px",
        ...
        {/* Microphone toggle button (keyboard accessible) */}
        <button
          ref={micButtonRef}
